...
Rails integrates some mailing functionalites. Let's implement a welcome email when a user registers to our blog:
...
Now let's customize `UserMailer#welcome`. It works in a similar way than a controller in the sense that it will render an HTML view that has the same name as the method. And the instance variables are available in the view:
...
We can now define the email content by customizing the views. It's important to implement both the HTML and the text version for email clients who dont' support HTML emails:
