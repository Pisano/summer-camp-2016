...
We are going to learn Rails by building an example app.

As always, the [Documentation](http://guides.rubyonrails.org/v4.2.6/) is our best friend.

## Rails Basics

Let's say we want to create a simple blog with these specs:
...
- Users will also be able to see comments on posts, write comments, edit their comments and delete them.
...
```sh
$ rails server # or, thanks to our dotfiles: rails s
```
...
```sh
$ rails console # or, thanks to our dotfiles: rails c
```
...
By default, Rails controllers render HTML views. At Pisano, our Rails application communicates with other apps, not with humans. Humans like HTML, but computers prefer another format: JSON. We will talk later about rendering JSON files.

### Rails Model

All this is nice for static content. But how are we going to manage our blog posts and comments? They will be created by the app's users, and we will have to store them in the the database.
...
By convention, a model name is singular (And CamelCase as in Ruby convention), and it corresponds to it's snake_base pluralized table name in the database: Model `Post` corresponds to the `posts` table in the database.

<!-- #### ActiveRecord in Rails -->
<!-- TODO: either describe ActiveRecord or reference to the course it's in. -->

We can run all the pending migrations (only one in our case) to update the database by running this terminal command:

```sh
rake db:migrate
```

<!-- Should the following be in ActiveRecord? -->
This created a new file in `db/schema.rb` because it's our first migration. This file will be updated after every migration.

<!-- This should be in active record

If we want to rollback the last migration, we can run:
...
- We probably don't want to list all the comments. It would make no sense without the relative Post. However, let's say that we want a page with all the comments of a single post. So we still want an `index` method.
- We probably don't want to show a single comment, it would make no sense without context.
- To keep it simple, we will keep the standards `new`/`create` methods.
- To keep it simple, let's say that a User can't edit or destroy a comment (it would be the same logic as for posts).

```sh
rails generate controller Comments index new create
```

As the comments are very dependent on their relative post, we define the routes as nested resources:
